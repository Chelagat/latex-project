...
if __name__ == "__main__":
  ...
  #  print("Creating testing dataset")
    train_dataset = make_dataset(mnist.train, 100, 2)
    print len(train_dataset['labels'])
    X_scale = StandardScaler()
    array = np.array(train_dataset['examples'])
    X = X_scale.fit_transform(train_dataset['examples'])
    print X.shape
    ...
    nn_structure = [1568, 30, 100]
    W, b, avg_cost_func = train_nn(nn_structure, X_train, y_train)
    '''
    for i in range(len(train_dataset['labels'])):
        #  print "Label: {}".format(train_dataset['labels'][0])
        example = train_dataset['examples'][i]
        plt.imshow(example, cmap='gray')
        plt.show()
        label = train_dataset['labels'][i]
        digits = np.nonzero(label)
        print digits, example.shape
    '''
